...
local function is_growth_building(building_name)
     return (string.find(building_name, "growth") or string.find(building_name, "farm") or string.find(building_name, "wh_main_brt_hospital")) and
...
local function Growth_Listener()
     ...
     core:add_listener("AI_CPR_DismantleGrowthListener", "BuildingCompleted", function(context)
          ...
          if not region_owner:is_human() then
               ...
               for _, current_region in model_pairs(province:regions()) do
                    if current_region then
                         out("Inspecting region: " .. current_region:name() .. ", owned by faction: " .. current_region:owning_faction():name())
